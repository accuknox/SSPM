"""CIS Azure 6.1.2.8 – Ensure that Activity Log Alert Exists for Delete SQL Server Firewall Rule (Automated, L1)"""
...
_OPERATION = "microsoft.sql/servers/firewallrules/delete"


@registry.rule
class CIS_6_1_2_8(AzureRule):
    metadata = RuleMetadata(
        id="azure-cis-6.1.2.8",
        ...
        description=(
            "An activity log alert must exist for the Delete SQL Server Firewall Rule operation "
            "to notify administrators when firewall rules are removed from SQL Servers."
        ),
        rationale=(
            "Deletion of SQL Server firewall rules could expose the database to unrestricted "
            "network access or indicate that an attacker has removed IP restrictions to maintain "
            "persistent access. Alerting ensures timely detection."
        ),
        ...
        remediation=(
            ...
            "Signal = SQL Server Firewall Rules Delete → Create action group → Create."
        ),
