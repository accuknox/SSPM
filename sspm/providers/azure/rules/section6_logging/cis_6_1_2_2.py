"""CIS Azure 6.1.2.2 – Ensure that Activity Log Alert exists for Delete Policy Assignment (Automated, L1)"""
...
_OPERATION = "microsoft.authorization/policyassignments/delete"


@registry.rule
class CIS_6_1_2_2(AzureRule):
    metadata = RuleMetadata(
        id="azure-cis-6.1.2.2",
        ...
        description=(
            "An activity log alert must exist for the Delete Policy Assignment operation to "
            "notify administrators when policy assignments are removed."
        ),
        rationale=(
            "Deletion of policy assignments can silently remove security controls from the "
            "subscription. Alerting on this operation ensures that unauthorized removals are "
        ...
        remediation=(
            ...
            "Signal = Policy Assignment Delete → Create action group → Create."
        ),
