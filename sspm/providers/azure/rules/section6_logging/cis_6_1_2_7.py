"""CIS Azure 6.1.2.7 – Ensure that Activity Log Alert Exists for Create or Update SQL Server Firewall Rule (Automated, L1)"""
...
_OPERATION = "microsoft.sql/servers/firewallrules/write"


@registry.rule
class CIS_6_1_2_7(AzureRule):
    metadata = RuleMetadata(
        id="azure-cis-6.1.2.7",
        ...
        description=(
            "An activity log alert must exist for the Create or Update SQL Server Firewall Rule "
            "operation to notify administrators of changes to SQL Server network access controls."
        ),
        rationale=(
            "SQL Server firewall rules control which IP addresses can connect to the database "
            "server. Unauthorized modifications could expose the database to unauthorized access. "
            "Alerting on firewall rule changes enables rapid detection and response."
        ),
        ...
        remediation=(
            ...
            "Signal = SQL Server Firewall Rules Create or Update → Create action group → Create."
        ),
