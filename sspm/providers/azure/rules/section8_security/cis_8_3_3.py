"""CIS Azure 8.3.3 – Ensure that the Expiration Date is set for All Secrets in Key Vaults using RBAC (Automated, L1)"""
...
@registry.rule
class CIS_8_3_3(AzureRule):
    metadata = RuleMetadata(
        id="azure-cis-8.3.3",
        title="Ensure that the Expiration Date is set for All Secrets in Key Vaults using RBAC",
        ...
        description=(
            "All enabled secrets stored in RBAC-enabled Azure Key Vaults must have an "
            "expiration date set to ensure that stale credentials and tokens are rotated "
        ...
        audit_procedure=(
            "ARM: list secrets for each RBAC-enabled Key Vault — every enabled secret must "
    ...
    async def check(self, data: CollectedData) -> "Finding":
        ...
        if offenders:
            return self._fail(
                f"{len(offenders)} secret(s) in RBAC vaults lack an expiry date: "
                f"{', '.join(offenders[:5])}.",
                evidence=evidence,
            )
        return self._pass(
            "All enabled secrets in RBAC Key Vaults have an expiry date set.",
            evidence=evidence,
        )
