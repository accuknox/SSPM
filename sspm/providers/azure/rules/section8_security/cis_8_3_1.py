"""CIS Azure 8.3.1 – Ensure that the Expiration Date is Set for all Keys in Key Vaults using RBAC (Automated, L1)"""
...
@registry.rule
class CIS_8_3_1(AzureRule):
    metadata = RuleMetadata(
        id="azure-cis-8.3.1",
        title="Ensure that the Expiration Date is Set for all Keys in Key Vaults using RBAC",
        ...
        description=(
            "All enabled cryptographic keys stored in RBAC-enabled Azure Key Vaults must have "
            "an expiration date set to ensure that stale or potentially compromised keys are "
            "rotated on a scheduled basis."
        ),
        rationale=(
        ...
        audit_procedure=(
            "ARM: list keys for each RBAC-enabled Key Vault — every enabled key must have "
    ...
    async def check(self, data: CollectedData) -> "Finding":
        ...
        offenders: list[str] = []
        for vault in vaults:
            if not vault.get("properties", {}).get("enableRbacAuthorization"):
                continue  # Only check RBAC-enabled vaults
            vid = vault.get("id", "")
        ...
        if offenders:
            return self._fail(
                f"{len(offenders)} key(s) in RBAC vaults lack an expiry date: "
                f"{', '.join(offenders[:5])}.",
                evidence=evidence,
            )
        return self._pass(
            "All enabled keys in RBAC Key Vaults have an expiry date set.",
            evidence=evidence,
        )
